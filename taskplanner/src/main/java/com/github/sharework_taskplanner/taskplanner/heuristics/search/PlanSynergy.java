 ...
 *
 */
public class PlanSynergy 
{
	public static Map<ComponentValue, List<List<ComponentValue>>> pgraph;		// planning graph
	public static DomainComponent robot;
	public static DomainComponent human;
	public static double SYN_MAX;
	 ...
	 * @param collection
	 */
	public PlanSynergy(SearchSpaceNode node, MongoCollection<Document> collection) {//double pRisk, double hRisk) {
		// set collection 
	 ...
	 * @return
	 */
	public double getHeuristicSynergy() {
				...
				for (DecisionVariable rTask : rTasks) 
				{
					...
					for (DecisionVariable hTask : hOverTasks) 
					{
						...
						Document record = this.collection.find(Filters.and(
								Filters.eq("human_right_arm_skill", hTaskName), 
								Filters.eq("ur5_on_guide_skill", rTaskName))).first();
...
//							Object riskValue = record.get("risk-dynamic");
							Object riskValue = record.get("dynamic_risk");
							...
							// set maximum risk value as there is no information concerning the combination
							rTaskRisk += SYN_MAX;
						}
					}
